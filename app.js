...
const MASTER_TABLE_DATA = [
  ...
  { day: "DAY 18", date: "4/07 (수)", city: "포르투 ➔ 리스본", morning: "포르투 캄파냐 역 ➔ 리스본 AP 고속열차 이동 (약 3시간 소요)", afternoon: "리스본 바이샤-시아두 호텔 체크인 ➔ 28번 노랑 트램 알파마 코스 ➔ 상 조르제 성 언덕 일몰", transport: "🚆 AP 고속열차 (약 3h)", stay: "리스본 4성급 (1/3박)", transportType: "train" },
...
const ITINERARY = [
  ...
  },
  {
    ...
    tl: [
      ...
      { t:"오후", d:"포르투 역사 카페 '카페 마제스틱' ➔ 자유 쇼핑 & 도루강변 오션뷰 레스토랑 로맨틱 디너" },
...
/* ── Interactive Map Layer Populate (Semi-transparent White Box Background Readability) ── */
function populateMapLayers(mapObj) {
  ...
  pins.forEach(pin => {
    ...
    // 인라인 흰색 반투명 30% 박스 (rgba(255, 255, 255, 0.70))
    const labelHtml = `<div style="background: rgba(255, 255, 255, 0.70) !important; color: #FF0000 !important; border: 2.5px solid #FF0000 !important; padding: 4px 10px !important; border-radius: 20px !important; font-weight: 900 !important; font-size: 0.92rem !important; box-shadow: 0 4px 14px rgba(0,0,0,0.35) !important; backdrop-filter: blur(4px) !important; white-space: nowrap !important; display: inline-block !important;">${pin.labelText}</div>`;
  ...
  // 3. 🛤️ 실제 고속열차 & 도로망 Waypoint 좌표를 따라 이동하는 파란색 노선 선 (#1d4ed8)
  const iberiaRoutes = [
    {
  ...
  iberiaRoutes.forEach(r => {
    ...
    if (r.midPoint && r.timeLabel) {
      // 인라인 흰색 반투명 30% 박스 (rgba(255, 255, 255, 0.70))
      const timeLabelHtml = `<div style="background: rgba(255, 255, 255, 0.70) !important; color: #0000FF !important; border: 2px solid #0000FF !important; padding: 3px 9px !important; border-radius: 12px !important; font-weight: 900 !important; font-size: 0.86rem !important; box-shadow: 0 4px 14px rgba(0,0,0,0.35) !important; backdrop-filter: blur(4px) !important; white-space: nowrap !important; display: inline-block !important;">${r.timeLabel}</div>`;
